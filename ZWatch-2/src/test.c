...
void getRfBuffer(INT8U  *buf)
{
 ...
 // rfBuffer.sum=CRC16(data,12);
  ...
  rfBuffer.macID=*(data++);
  for (i=0;i<8;i++)
    rfBuffer.tableID[i]=*(data++);
    for (i=0;i<4;i++)
    rfBuffer.orderID[i]=*(data++);
    ...
    int main( void )
{
  ...
  //  UartSendString("zwatch",6);
    ...
    //  UartSendString("rf recv",8);
      len = rf_rec_packet(rbuffer, &rssi, &lqi, 240) ;
    if(PWR_KEY==PWR_DOWN)
       {
         keyCount++;
         keyFlag=1;
      ...
      UartSendString((uchar *)rbuffer,len);
         if(len!=0)
      {
     // UartSendString((uchar *)len,1);  
     //   getRfBuffer(buffer);
      // UartSendString((uchar *)rbuffer,14);
     //   UartSendString((uchar *)&rbuffer,len);
        timeCount=0;
        ledBuffer.led0=rfBuffer.macID/10;
        ledBuffer.led1=rfBuffer.macID%10;
      ...
      getRfBuffer(rbuffer);
            ...
            if(rfBuffer.macID==MYADDR&&rfBuffer.msgType==MSG_TIME)
            {
               ...
               if(keyFlag==1)
              {
                ...
                rf_send_packet((INT8U *) &rfBuffer, BUFFER_SIZE);//发送应答信号   s
              }  
            }else
              showRGB(NO_VAL);
